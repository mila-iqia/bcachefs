...
#define BCH_SB_FIELDS()                                                                                                \
    x(journal, 0) x(members, 1) x(crypt, 2) x(replicas_v0, 3) x(quota, 4) x(disk_groups, 5) x(clean, 6) x(replicas, 7) \
        x(journal_seq_blacklist, 8)

#define BCH_JSET_ENTRY_TYPES()                                                                                        \
    x(btree_keys, 0) x(btree_root, 1) x(prio_ptrs, 2) x(blacklist, 3) x(blacklist_v2, 4) x(usage, 5) x(data_usage, 6) \
        x(clock, 7) x(dev_usage, 8)

    /* Btree: */

#define BCH_BTREE_IDS() \
    x(extents, 0) x(inodes, 1) x(dirents, 2) x(xattrs, 3) x(alloc, 4) x(quotas, 5) x(stripes, 6) x(reflink, 7)
...
#define BCH_BKEY_TYPES()                                                                                          \
    x(deleted, 0) x(discard, 1) x(error, 2) x(cookie, 3) x(hash_whiteout, 4) x(btree_ptr, 5) x(extent, 6)         \
        x(reservation, 7) x(inode, 8) x(inode_generation, 9) x(dirent, 10) x(xattr, 11) x(alloc, 12) x(quota, 13) \
            x(stripe, 14) x(reflink_p, 15) x(reflink_v, 16) x(inline_data, 17) x(btree_ptr_v2, 18)                \
                x(indirect_inline_data, 19) x(alloc_v2, 20)

#define BCH_EXTENT_ENTRY_TYPES() x(ptr, 0) x(crc32, 1) x(crc64, 2) x(crc128, 3) x(stripe_ptr, 4)
...
#define BCH_KEY_MAGIC                                                                              \
    (((uint64_t)'b' << 0) | ((uint64_t)'c' << 8) | ((uint64_t)'h' << 16) | ((uint64_t)'*' << 24) | \
     ((uint64_t)'*' << 32) | ((uint64_t)'k' << 40) | ((uint64_t)'e' << 48) | ((uint64_t)'y' << 56))
...
#undef x
    ...
    static const struct u64s_spec U64S_BCH_SB_FIELD = {.size = sizeof(uint32_t), .start = 0};
    static const struct u64s_spec U64S_JSET_ENTRY   = {.size = sizeof(uint16_t), .start = 1};
    static const struct u64s_spec U64S_BKEY_I       = {.size = sizeof(uint8_t), .start = 0};
    static const struct u64s_spec U64S_BKEY         = {.size = sizeof(uint8_t), .start = 0};
    ...
    static const struct bkey_format BKEY_FORMAT_SHORT = {.key_u64s       = 3,
                                                         .nr_fields      = 6,
    ...
    struct bkey_short
    {
        ...
        uint8_t format : 7, needs_whiteout : 1;
    ...
    {
        uint32_t type : 2, _compressed_size : 7, _uncompressed_size : 7, offset : 7, _unused : 1, csum_type : 4,
            compression_type : 4;
        uint32_t csum;
    } __attribute__((packed, aligned(8)));
...
#define CRC32_NONCE_MAX 0
    ...
    {
        uint64_t type : 3, _compressed_size : 9, _uncompressed_size : 9, offset : 9, nonce : 10, csum_type : 4,
            compression_type : 4, csum_hi : 16;
        uint64_t csum_lo;
    } __attribute__((packed, aligned(8)));
...
#define CRC64_NONCE_MAX ((1U << 10) - 1)
    ...
    {
        uint64_t type : 4, _compressed_size : 13, _uncompressed_size : 13, offset : 13, nonce : 13, csum_type : 4,
            compression_type : 4;
        struct bch_csum csum;
    } __attribute__((packed, aligned(8)));
...
#define CRC128_NONCE_MAX ((1U << 13) - 1)
    ...
    {
        uint64_t type : 1, cached : 1, unused : 1, reservation : 1, offset : 44, /* 8 petabytes */
            dev : 8, gen : 8;
    } __attribute__((packed, aligned(8)));

    struct bch_extent_stripe_ptr
    {
        uint64_t type : 5, block : 8, redundancy : 4, idx : 47;
    };

    struct bch_extent_reservation
    {
        uint64_t type : 6, unused : 22, replicas : 4, generation : 32;
    };
...
#define BCACHEFS_ROOT_INO 4096
    ...
    static inline uint64_t __jset_magic(struct bch_sb* sb) { return __bch2_sb_magic(sb) ^ JSET_MAGIC; }

    static inline uint64_t __bset_magic(struct bch_sb* sb) { return __bch2_sb_magic(sb) ^ BSET_MAGIC; }

    static const struct uuid BCACHE_MAGIC = {
        {0xc6, 0x85, 0x73, 0xf6, 0x4e, 0x1a, 0x45, 0xca, 0x82, 0x65, 0xf5, 0x7f, 0x48, 0xba, 0x6d, 0x81}};

    const void* benz_bch_next_sibling(const void* p, uint32_t sizeof_p, const void* p_end, const void* c,
                                      struct u64s_spec u64s_spec);

    const struct bch_sb_field* benz_bch_next_sb_field(const struct bch_sb* p, const struct bch_sb_field* c,
                                                      enum bch_sb_field_type type);
    const struct jset_entry*   benz_bch_next_jset_entry(const struct bch_sb_field* p, uint32_t sizeof_p,
                                                        const struct jset_entry* c, enum bch_jset_entry_type type);
    ...
    const struct bset*         benz_bch_next_bset(const struct btree_node* p, const void* p_end, const struct bset* c,
                                                  const struct bch_sb* sb);
    ...
    const struct bkey* benz_bch_file_offset_size(const struct bkey* bkey, const struct bch_val* bch_val,
                                                 uint64_t* file_offset, uint64_t* offset, uint64_t* size);
    uint64_t           benz_bch_inline_data_offset(const struct btree_node* start, const struct bch_val* bch_val,
    ...
    uint64_t benz_bch_fread_btree_node(struct btree_node* btree_node, const struct bch_sb* sb,
                                       const struct bch_btree_ptr_v2* btree_ptr, FILE* fp);
