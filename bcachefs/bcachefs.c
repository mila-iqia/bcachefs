...
//
// NOTE: benz_bch_next_sibling is currently used in 3 locations and everytime
// p_end is `p + p->u64s * BCH_U64S_SIZE`
//
...
//
// TODO: sizeof_p is always sizeof(struct bch_sb_field_clean) we could simplify
// this function by removing its argument or/and change p type to
// `bch_sb_field_clean`
...
// c       : is our child. Currently always a btree_ptr
// sizeof_c: is the size of child. Currently `struct bch_btree_ptr_v2`
...
const struct bch_val *BCacheFS_iter_next(const BCacheFS *this, BCacheFS_iterator *iter)
{
    ...
    // if next_it is a reverse list we only need to do this once then fetch the
    // key, if the key is null delete current next_it and check next one in the
    // stack
...
BCacheFS_extent BCacheFS_iter_make_extent(const BCacheFS *this, BCacheFS_iterator *iter)
{
    ...
    // TODO: this is O(d) with d the depth if the list is in reverse we could
    // simply use next_it
    while (iter->next_it)
    {
        iter = iter->next_it;
...
BCacheFS_dirent BCacheFS_iter_make_dirent(const BCacheFS *this, BCacheFS_iterator *iter)
{
    ...
    // this is O(d) with d the depth if the list is in reverse we could simply
    // use next_it
    while (iter->next_it)
    {
        iter = iter->next_it;
