...
{
    BCacheFS bchfs = {0};
    if (BCacheFS_open(&bchfs, "img/2021-08-09-proto_bcachefs.img")) {}
    else
    {
        return 1;
    }
                                                             ...
                                                             BCH_JSET_ENTRY_btree_root))
    {
        ...
        bch_val = benz_bch_next_bch_val(&bkey_i->k, NULL, 0);
            ...
            for (; bset; bset = benz_bch_next_bset(btree_node, bset, sb))
            {
                switch (jset_entry->btree_id)
                {
                    ...
                    case BTREE_ID_dirents:
                    {
                        ...
                        const struct bkey_dirent *bkey_dirent = benz_bch_next_bkey_dirent(bset, NULL, inode);
                        const struct bkey_dirent *last_bkey_dirent = NULL;
                        while (bkey_dirent)
                        {
                            last_bkey_dirent = bkey_dirent;
                            const struct bch_dirent *bch_dirent = benz_bch_dirent(bkey_dirent);
                            const char *full_path = benz_bch_strcpy_file_full_path(buffer + sizeof(buffer), bset, bkey_dirent);
                            ...
                            {
                                bkey_dirent = benz_bch_next_bkey_dirent(bset, NULL, bch_dirent->d_inum);
                            }
                            else
                            {
                                bkey_dirent = benz_bch_next_bkey_dirent(bset, bkey_dirent, bkey_dirent->p.inode);
                            }

                            while (bkey_dirent == NULL && last_bkey_dirent)
                            {
                                bkey_dirent = benz_bch_parent_bkey_dirent(bset, last_bkey_dirent);
                                last_bkey_dirent = bkey_dirent;
                                if (bkey_dirent)
                                {
                                    bkey_dirent = benz_bch_next_bkey_dirent(bset, bkey_dirent, bkey_dirent->p.inode);
